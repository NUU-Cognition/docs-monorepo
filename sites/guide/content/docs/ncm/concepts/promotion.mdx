...
A block is not only an element with an identity. Most elements map one to one. Promotion is the extra step that builds a richer meaning. See [Concept - Core Ideas](/ncm/concepts/core-ideas).
...
Identity is assigned after promotion. The new block gets a positional identity and a stable identity. See [Concept - Block Identity](/ncm/concepts/block-identity).
...
Promotion also does not run unless the matching extension is on. A CommonMark or GFM preset leaves these patterns as ordinary quotes, fences, and images. See [Concept - Presets and Extensions](/ncm/concepts/presets-and-extensions).
...
You get a tree of typed blocks. A renderer walks that tree. Each type can have its own drawing. A bad block can fall back without taking down the rest of the document. See [Guide - Parse and Render a Document](/ncm/guides/parse-and-render-a-document).

To write the patterns, follow [Guide - Use Callouts Diagrams and Figures](/ncm/guides/use-callouts-diagrams-and-figures).
