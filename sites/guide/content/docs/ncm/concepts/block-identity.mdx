...
Identity is why NCM is not only Markdown as a string. The same input always produces the same identities. That property is deterministic. See [Core Ideas](/ncm/concepts/core-ideas).
...
The parser assigns identity when it builds blocks. You do not assign positional identity by hand. You can assign a stable identity with an author anchor. See [Promotion](/ncm/concepts/promotion) for how identity is attached after promotion.
