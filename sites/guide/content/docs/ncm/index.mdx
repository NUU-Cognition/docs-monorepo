...
NCM is the format and the parse. A renderer is a separate step. See [Guide - Parse and Render a Document](/ncm/guides/parse-and-render-a-document).

## What you get

You get a document with metadata, elements, and blocks. See [Concept - Core Ideas](/ncm/concepts/core-ideas).

You get two kinds of identity on every block. One is positional. One is stable. See [Concept - Block Identity](/ncm/concepts/block-identity).

You get promotion. Promotion turns a familiar Markdown pattern into a richer block. See [Concept - Promotion](/ncm/concepts/promotion).

You get presets. A preset is a named set of parse choices. CommonMark, GFM, and NCM are the main presets. You can switch extensions on or off. See [Concept - Presets and Extensions](/ncm/concepts/presets-and-extensions).

Frontmatter carries metadata such as title and tags. See [Guide - Add Metadata with Frontmatter](/ncm/guides/add-metadata-with-frontmatter).
...
If you want the mental model first, read [Concept - Core Ideas](/ncm/concepts/core-ideas).

If you want to write a document now, follow [Guide - Write Your First NCM Document](/ncm/guides/write-your-first-ncm-document).

If you already have Markdown, follow [Guide - Move Existing Markdown to NCM](/ncm/workflows/move-existing-markdown-to-ncm).
