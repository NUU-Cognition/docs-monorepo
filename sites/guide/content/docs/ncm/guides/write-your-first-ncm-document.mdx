...
- You will parse with the default NCM preset. See [Presets and Extensions](/ncm/concepts/presets-and-extensions).
...
Tags without a hash get a hash when the document is parsed. You can also write the hash yourself. See [Add Metadata with Frontmatter](/ncm/guides/add-metadata-with-frontmatter).
...
The heading gets a slug from its text. The paragraph gets the stable identity `lead`. See [Block Identity](/ncm/concepts/block-identity).
...
To embed media, use a Markdown image line as its own block. Images are not inline. See [Use Callouts Diagrams and Figures](/ncm/guides/use-callouts-diagrams-and-figures).
...
That tree is the document that a renderer draws. See [Parse and Render a Document](/ncm/guides/parse-and-render-a-document).
