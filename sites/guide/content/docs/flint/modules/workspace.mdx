...
flint workspace clone "App" git@github.com:me/app.git
flint workspace register "App"
...
- `create` records a repository declaration in `flint.toml`; the kernel can materialize the missing clone on sync or plan-apply
- `clone` clones an existing remote into `Workspace/Repos/` and records the declaration
- `register` records an existing git repository already under `Workspace/Repos/`, using its `origin` remote
- `update` pulls the latest changes if the repo exists, or clones it if the local folder is missing
- `remove` removes the declaration from config and leaves the folder on disk for manual cleanup
