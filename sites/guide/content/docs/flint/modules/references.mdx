...
The core reference feature supports four reference slices:
...
They are declared in `flint.toml` under `[references]`. The public `flint reference` command currently declares codebases and Flints directly; lattice and bundle references are driven by their own workflows or by TOML declarations.
...
```bash
flint reference codebase "Flint" /Users/me/dev/flint
flint fulfill codebase "Flint" /Users/me/dev/flint
flint resolve codebase "Flint"
```

`flint reference codebase` declares the reference and records the local path for this machine. `flint fulfill codebase` is still useful for codebase references that were declared by config, Tinderbox wiring, or another machine but are not fulfilled locally yet.
...
Each fulfillment records a name, local path, and optional fulfillment timestamp.
...
flint resolve codebase "Flint"
flint resolve flint "Research Hub"
flint resolve lattice "customer_data"
flint resolve bundle "Research Hub/Guide"
flint resolve source "Research Hub/Guide"
```
