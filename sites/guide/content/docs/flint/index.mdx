...
Your own ideas, plans, and decisions go in `Mesh/`. External source material comes in through `Sources/` or references. Active repo clones live in `Workspace/Repos/`. Shareable output gets built into `Exports/`. Local Flint state stays in `.flint/`, while Orbh agent session spools live in `.orb/`.
...
| `Sources/` | Read-only external material copied into the workspace, including `Sources/Repos/` and `Sources/Bundles/` |
| `Exports/` | Built, shareable outputs generated from Mesh |
| `Workspace/` | Active repositories and workspace-side working surfaces, especially `Workspace/Repos/` |
| `.flint/` | Local Flint state such as references, lattices, and sync bookkeeping |

When you use Orbh agent sessions, Flint also creates `.orb/` for session spools, runs, messages, and indexes.
...
- source repositories
- source bundles from Flint exports
...
1. [Guide - Quick Start](/flint/guide/quick-start)
2. [Guide - Core Concepts](/flint/guide/core-concepts)
3. [Guide - Creating a Flint](/flint/guide/creating-a-flint)

If you already understand the basic model and want to do real work:

- [Guide - Tutorial Project Planning](/flint/guide/tutorial-project-planning)
- [Guide - Tutorial Research Notes](/flint/guide/tutorial-research-notes)
- [Guide - Tutorial Agent Collaboration](/flint/guide/tutorial-agent-collaboration)
