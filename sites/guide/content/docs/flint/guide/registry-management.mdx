...
That identity is stored in the shared `~/.nuucognition/config.toml` and resolves to `Mesh/People/@<Name>.md` in each workspace. It is a different layer of state from the global registry.
