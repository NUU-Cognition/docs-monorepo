...
- runs the initial sync and kernel reconciliation pass
...
`blank` is the implicit default if you do not specify a preset. It keeps the workspace light while still installing the core Flint and Orbh shards. `default` installs a broader starter set, including agent, planning, notepad, reports, increments, and projects shards.
...
- generate metadata markers for sources and references
- synchronize source repositories and source bundles
- update `.gitignore`
...
```bash
flint reference codebase "Main App" /path/to/app
```
