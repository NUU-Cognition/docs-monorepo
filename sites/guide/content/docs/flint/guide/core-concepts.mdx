...
- export builds
- shard workflows
- init-note driven Orbh sessions
- MCP tools exposed by `flint mcp serve`
...
- `.flint/plates/` for local Steel Plate state
...
That same pattern appears again with lattices, source bundles, and some sync-managed relationships. Once you notice it, Flint becomes much easier to reason about.
...
- a sourced bundle export from another Flint in `Sources/Bundles/`
...
3. build it into `Exports/(Bundle) <Name>/`
...
- an Orb-backed session spool under `.orb/`
- work states
- runs
- request/response records
- interface state such as `phase`, `progress`, and `artifacts`
- return results

That means AI work is not just “something that happened in a terminal once.” It can become inspectable workspace state.

## 10. Server and MCP Are Optional Layers

The server and MCP surfaces are important, but they are not the center of Flint.

- the server provides a local HTTP control plane
- `flint mcp serve` exposes Flint workspace capabilities to MCP clients

If you do not need automation or external tool integration, you can still use Flint effectively without leaning on them. They are optional power layers, not the base product.
