...
```bash
flint reference codebase "Payments API" /Users/me/dev/payments-api
```
...
installed = [
  ...
  { name = "increments", remote = "NUU-Cognition/shard-increments" },
  { name = "plan", remote = "NUU-Cognition/shard-plan" },
]
```
...
- links to the codebase reference note such as `[rf-cb-payments_api](/flint/rf-cb-payments-api)`
...
- new shards
- more references
- a new source bundle from another Flint export
...
`flint git sync` is useful because it runs `flint sync` before the git flow, then protects against overwriting remote work. That helps keep committed state aligned with declared state.
