...
- `Sources/Repos/` and `Sources/Bundles/` are for copied-in external context.
- `Workspace/Repos/` is for active repos that belong inside the workspace.
- `Exports/` is for built output.
- `.flint/` is for local fulfillment and sync state.
...
- start linking related notes with `[wiki links](/flint-server/wiki-links)`
...
Add a source bundle from another Flint export:
...
- [Guide - Tutorial Project Planning](/flint-server/guide-tutorial-project-planning)
- [Guide - Tutorial Research Notes](/flint-server/guide-tutorial-research-notes)
- [Guide - Tutorial Agent Collaboration](/flint-server/guide-tutorial-agent-collaboration)
